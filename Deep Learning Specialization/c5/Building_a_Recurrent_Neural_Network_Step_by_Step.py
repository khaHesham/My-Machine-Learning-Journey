...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    ft = sigmoid(np.dot(Wf,concat)+bf)                   #𝚪⟨𝑡⟩𝑓=𝜎(𝐖𝑓[𝐚⟨𝑡−1⟩,𝐱⟨𝑡⟩]+𝐛𝑓)
    it = sigmoid(np.dot(Wi,concat)+bi)                   #𝚪⟨𝑡⟩𝑖=𝜎(𝐖𝑖[𝑎⟨𝑡−1⟩,𝐱⟨𝑡⟩]+𝐛𝑖)
    cct = np.tanh(np.dot(Wc,concat)+bc)                  #𝐜̃ ⟨𝑡⟩=tanh(𝐖𝑐[𝐚⟨𝑡−1⟩,𝐱⟨𝑡⟩]+𝐛𝑐)
    c_next =ft*c_prev+it*cct                             #𝐜⟨𝑡⟩=𝚪⟨𝑡⟩𝑓∗𝐜⟨𝑡−1⟩+𝚪⟨𝑡⟩𝑖∗𝐜̃ ⟨𝑡⟩
    ot = sigmoid(np.dot(Wo,concat)+bo)                   #𝚪⟨𝑡⟩𝑜=𝜎(𝐖𝑜[𝐚⟨𝑡−1⟩,𝐱⟨𝑡⟩]+𝐛𝑜)
    a_next = ot*np.tanh(c_next)                          #𝐚⟨𝑡⟩=𝚪⟨𝑡⟩𝑜∗tanh(𝐜⟨𝑡⟩)
    
    # Compute prediction of the LSTM cell (≈1 line)
    yt_pred = softmax(np.dot(Wy,a_next)+by)              #𝐲⟨𝑡⟩𝑝𝑟𝑒𝑑=softmax(𝐖𝑦𝐚⟨𝑡⟩+𝐛𝑦)
    ### END CODE HERE ###
